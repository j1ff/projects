...
int main(int argc, char *argv[])
{
	...
	int byteCount = 0;
	wordListData = malloc(1024); // 1k bytes to store words..
	...
	while (wordIndex < MAXWORDS)
	{
 		...
 		*/
 	// New way //
 		// need a pointer into big chunk of memory, to next unused byte
 		// wordListData + count of bytes used so far
 		// initially byte count is 0
 		
 		wordListP[wordIndex] = wordListData + byteCount;
